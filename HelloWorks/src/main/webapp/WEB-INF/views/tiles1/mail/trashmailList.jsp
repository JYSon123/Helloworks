...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		// 메일 선택살리기(휴지통)
		$("button#reviveTrash").click(function(){
	...
	function goSearch() {
		...
		frm.method = "GET";
		frm.action = "<%= request.getContextPath()%>/trashmailList.hello2";
		frm.submit();
...
<div class="w3-main" style="margin:30px 0 0 300px" >
  ...
  <div class="w3-container w3-padding-large" style="margin-bottom: 300px;">
	...
	<h2 style="margin-bottom: 30px;">휴지통</h2>
	
	
	
	
	<div style="width: 110%;">
		<button type="button" class="btn btn-secondary btn-sm" id="reviveTrash" style="float: left;">복구</button>&nbsp;&nbsp;
		<button type="button" class="btn btn-secondary btn-sm" id="deleteTrash" style="float: left; margin-left: 10px;">삭제</button>
		<%-- 메일검색 폼 추가하기 : 메일제목, 보낸이로 검색을 하도록 한다. --%>
		<form name="searchFrm" style="margin-bottom: 20px; width: 80%; float: left; padding-left: 8% ">
			<select name="searchType" id="searchType" style="height: 26px;">
				<option value="mailsubject">메일제목</option>
				<option value="sendid">보낸이</option>
				<option value="recid">받는이</option>
			</select>
	...
	<table style="width: 900px" class="table table-bordered">
		...
		<tbody>
		<c:forEach var="mailvo" items="${requestScope.trashmailList}">
			<tr>
				<td style="width: 30px;">
			...
			<!-- 첨부파일이 있는 경우 끝 -->
				...
				<%-- <td align="left">${mailvo.mailsubject}</td> --%>
