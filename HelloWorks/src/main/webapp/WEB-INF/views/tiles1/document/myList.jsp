...
<div style="margin: auto; padding-left: 10%;">
   ...
   <h2 style="width: 1050px; margin-top:120px; margin-bottom: 20px;"><b>${sessionScope.loginEmp.empname}님의 기안 목록</b></h2>
   ...
   <table style="width: 1250px;" class="table table-bordered">
      ...
      <c:forEach var="documentList" items="${requestScope.documentList}">
         <tr>
            ...
            <c:if test="${not empty documentList.fileName}">
		        <span class="subject" onclick="goView('${documentList.doument_seq}')">${documentList.subject}</span> &nbsp;<img style="width: 15px;" src="<%= request.getContextPath()%>/resources/images/disk_jy.gif " />
	        </c:if>
            <%-- === 첨부파일이 있는 경우 끝 === --%>
     ...
     <button onclick="button()" class="btn btn-info btn-sm" style="margin-left: 10px; margin-bottom: 3px; background-color: #0070C0; color: white;" >검색</button>
