...
public class FileManager {
	...
	// +++ 파일 업로드하기 첫번째 방법 +++ : 속도가 비교적 느림.
	// byte[] bytes : 파일의 내용물
	// String originalFilename : 첨부된 파일의 원래 이름(확장자 필요해서)
	...
	public String doFileUpload(byte[] bytes, String originalFilename, String path) throws Exception {
		...
		// newFilename은 예를 들면, 2021110809271535243254235235234.png 와 같이 된다.
		...
		if(!dir.exists()) // 만약에 파일을 저장할 경로인 폴더가 실제로 존재하지 않는다면
			...
			// mkdir은 폴더 하나만 만드는 것이고, mkdirs는 만들어야 할 여러개의 폴더를 다 만든다.
	...
	public boolean doFileDownload(String saveFilename, String originalFilename, String path, HttpServletResponse response) {
		...
		// 운영체제가 UNIX 또는 Linux 이라면 File.separator 은 "/" 이다.
		// 해당경로에 \ 를 더하고 파일명을 더한 경로까지 나타내어준 파일명(문자열)을 만든다. 
		// pathname 은 예를 들면, C:\NCS\workspace(spring)\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\Board\resources\files\2021110809271535243254235235234.jpg
		...
		try {
			...
			if(file.exists()) { // 실제로 다운로드 할 해당 파일이 존재한다면
				...
				// 다운로드할 파일의 내용을 읽어오는 단위크기를 4096 byte(4KB)로 하는 byte 배열 readByte 를 생성한다. 
				...
				// ServletOutputStream 은 다운로드 되어질 파일을 클라이언트로 보내어주는 출력 스트림(웹 브라우저용)
				
				int length = 0;
				
				while((length = bfin.read(readByte, 0, 4096)) != -1) {
					// return 값은 실제로 한번에 읽어들인 크기를 말한다 ==> 예) 4096 -> 4096 -> 50 -> -1(이제 남은게 없다!)
					/*
						...
						다운로드 해주어야할 file 을 매번 4096 byte 씩 끝까지(-1) 읽어들인다는 말이다.         
					*/
				...
				return true; // 다운로드 해줄 파일이 존재하고, Exception이 발생하지 않으면 true를 리턴
	...
	public String doFileUpload(InputStream is, String originalFilename, String path) throws Exception {
		...
		fos.flush(); // 안적어도 됨(내부적으로 auto flush이기 때문에)
	...
	public File excelUpload(Map<String, Object> paraMap) throws Exception {
		
		Locale locale = (Locale) paraMap.get("locale");             // 넘겨받은 모델(다운로드되어질 엑셀파일의 정보)
		// Locale 클래스는 지역의 [언어][나라] 등의 정보를 담고 있는 클래스이다. 
	    
		String workbookName = (String) paraMap.get("workbookName"); // 넘겨받은 모델(다운로드되어질 엑셀파일의 정보)
		...
		try {
			workbook = (SXSSFWorkbook) paraMap.get("workbook");
			ost = new FileOutputStream(pathname);
