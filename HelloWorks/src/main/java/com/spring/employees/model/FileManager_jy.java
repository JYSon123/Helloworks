...
public class FileManager_jy {
	...
	public String doFileUpload(byte[] bytes, String originalFilename, String path) throws Exception { // 호출하는쪽에서 익셉션 처리를 해라
		...
		// pathname 은 예를 들면, 2021110809271535243254235235234.png 이다.
	...
	public boolean doFileDownload(String saveFilename, String originalFilename, String path, HttpServletResponse response ) {
		...
		try {
			...
			if(file.exists()) { // 실제로 다운로드할 해당 파일이 존재한다라면
				...
				while( (length = bfin.read(readByte, 0, 4096)) != -1 ) { // (읽어들일것을 저장할곳,  ,읽어들일 크기)
				/*
